...
export default function TiptapRenderer({ content }: TiptapRendererProps) {
  ...
  if (typeof content === "string") {
    try {
      ...
      return (
        <p className="leading-relaxed whitespace-pre-wrap text-zinc-800 dark:text-zinc-200">
          {content}
        </p>
      );
    }
  ...
  const renderMark = (textNode: TiptapNode, children: ReactNode, index: number) => {
    ...
    textNode.marks.forEach((mark, markIdx) => {
      ...
      switch (mark.type) {
        ...
        case "italic":
          result = (
            <em key={key} className="text-zinc-800 italic dark:text-zinc-200">
              {result}
            </em>
          );
          break;
        case "strike":
          result = (
            <span key={key} className="text-zinc-400 line-through dark:text-zinc-500">
              {result}
            </span>
          );
          break;
        ...
        case "code":
          result = (
            <code
              key={key}
              className="rounded bg-zinc-100 px-1.5 py-0.5 font-mono text-sm text-rose-500 dark:bg-zinc-800/60 dark:text-rose-400"
            >
              {result}
            </code>
          );
        ...
        case "link": {
          ...
          result = (
            <a
              ...
              className="text-indigo-600 underline decoration-indigo-400/50 transition-colors hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300"
            >
              {result}
            </a>
          );
  ...
  const renderNode = (node: TiptapNode, index: number): ReactNode => {
    ...
    switch (node.type) {
      case "paragraph":
        return (
          <p key={key} className="my-3 text-base leading-relaxed text-zinc-800 dark:text-zinc-200">
      ...
      case "bulletList":
        return (
          <ul
            key={key}
            className="my-4 list-disc space-y-1.5 pl-6 text-zinc-800 dark:text-zinc-200"
          >
            {node.content?.map((child, idx) => renderNode(child, idx))}
          </ul>
        );
      case "orderedList":
        return (
          <ol
            key={key}
            className="my-4 list-decimal space-y-1.5 pl-6 text-zinc-800 dark:text-zinc-200"
          >
            {node.content?.map((child, idx) => renderNode(child, idx))}
          </ol>
        );
      ...
      case "codeBlock": {
        ...
        return (
          <pre
            key={key}
            className="my-4 overflow-x-auto rounded-xl border border-zinc-200 bg-zinc-100/80 p-4 font-mono text-sm text-zinc-900 dark:border-zinc-800/80 dark:bg-zinc-900/50 dark:text-zinc-100"
          >
            <code>{codeText}</code>
          </pre>
        );
