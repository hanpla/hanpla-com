...
import { createPostAction, FormActionState } from "@/lib/actions/posts";
...
export const PostWriteForm = ({ boardAbbr, boardName }: PostWriteFormProps) => {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose dark:prose-invert prose-p:my-1.5 max-w-none focus:outline-none min-h-100 p-4 text-zinc-900 dark:text-zinc-100",
      },
    },
    onUpdate({ editor }) {
  ...
  // 등록 완료 시 상세 페이지로 리다이렉트
  useEffect(() => {
    if (state.success && state.postId) {
      router.push(`/board/${boardAbbr}/${state.postId}`);
    }
