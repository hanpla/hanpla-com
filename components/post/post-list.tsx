...
// 로컬 헬퍼 서브컴포넌트 (리스트 아이템) - 파일 상단 배치
const PostListItem = ({ post, showBoardBadge, postLink }: PostListItemProps) => {
  return (
    <Link
      href={postLink}
      className="group -mx-2 flex flex-col gap-1.5 rounded-lg px-2 py-3.5 text-sm transition-colors hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50"
    >
      {/* 1. Title Row */}
      <div className="flex min-w-0 items-center justify-between gap-4">
        <div className="flex min-w-0 items-center gap-1.5 font-medium text-zinc-900 dark:text-zinc-100">
          ...
          <span className="line-clamp-1 group-hover:underline">{post.title}</span>
        </div>
